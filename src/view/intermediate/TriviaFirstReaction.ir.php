...
class TriviaFirstReactionIR
{
   /*
   * Converts the array modelizing a piece of trivia into a string which corresponds to the first 
   * message that should be displayed in a topic containing reactions to such a piece of content.
   *
   * @param mixed $trivia[]  The array modelizing the piece
   * @return string          The first message of the topic containing reactions for this content
   */

   public static function process($trivia)
   {
      $truncatedText = strip_tags($trivia['content']);
      ...
      $output = '[g]'.$trivia['game'].': '.$trivia['title'].'[/g]'."\n";
      $output .= "\n";
      $output .= $truncatedText."\n";
      $output .= "\n";
      $output .= "[url=".PathHandler::triviaURL($trivia)."]Consulter l'anecdote dans son intégralité[/url]";
